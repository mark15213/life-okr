...
| 切到队列下一个任务 | Ctrl + Alt + N | Control + Option + N |
| 完成当前任务（自动切到下一个） | Ctrl + Alt + D | Control + Option + D |

切换器中用方向键选择、Enter 切换、⌘/Ctrl + ↑ 移到最前、⌘/Ctrl + Enter 完成当前任务、Esc 收起。任务列表就是队列：拖动行改变优先级；浮标下方常驻一条细栏，显示队列最前的三个任务（不含正在做的），点一下即切换；右端的图钉一键置顶并锁定位置。TickTick 任务完成时先调用云端接口，成功后才离开本地队列。Mac 使用 Control（⌃），不是 Command。设置中可以改绑；注册冲突会显示原因，鼠标入口仍可使用。
...
设置 → 输入已部署 Hustle 的根地址和看板解锁码 → 连接并同步。服务端须已有可用 TickTick 会话。默认地址是 `https://hustle-beta-i.vercel.app`。也可直接在完整看板输入解锁码；主窗口和任务面板共用登录会话。网页已有记录接口通过主进程白名单访问，不需要单独运行或部署后端。
