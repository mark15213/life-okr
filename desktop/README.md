...
切换器中用方向键选择、Enter 切换、⌘/Ctrl + Enter 完成当前任务、Esc 收起。任务列表就是队列：拖动行改变优先级；浮标下方常驻一条细栏，显示接下来的三个任务，点一下即切换。TickTick 任务完成时先调用云端接口，成功后才离开本地队列。Mac 使用 Control（⌃），不是 Command。设置中可以改绑；注册冲突会显示原因，鼠标入口仍可使用。
